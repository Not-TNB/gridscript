...
#[test]
fn segment_grazing_edge_misses() {
    let center = (5.0, 5.0);
    // closest approach is exactly `radius` — nodes are open discs, so this misses
    assert!(!segment_intersects_circle(
        (3.0, 4.0),
        (7.0, 4.0),
        center,
...
fn increment_defaults_to_one_and_to_the_dataspace() {
    ...
    inst.exec_command(&arith(ArithOp::Increment, None, None, None), &mut ctx)
        .unwrap();
...
fn arithmetic_operates_on_a_named_variable() {
    ...
    inst.exec_command(
        &arith(ArithOp::Decrement, Some(var("x")), Some(int(3)), None),
        &mut ctx,
    )
    .unwrap();
...
fn overflow_throws() {
    ...
    let result = inst.exec_command(
        &arith(ArithOp::Increment, Some(var("x")), Some(int(1)), None),
        &mut ctx,
    );
...
fn division_by_zero_throws() {
    ...
    inst.state.set_variable("x".into(), Value::Int(10));
    let result = inst.exec_command(
        &arith(ArithOp::Divide, Some(var("x")), Some(int(0)), None),
        &mut ctx,
    );
...
fn division_truncates_toward_zero() {
    ...
    inst.exec_command(
        &arith(ArithOp::Divide, Some(var("x")), Some(int(2)), None),
        &mut ctx,
    )
    .unwrap();
...
fn push_and_peek_are_fifo() {
    ...
    inst.exec_command(&Command::Push(Some(int(1))), &mut ctx)
        .unwrap();
    inst.exec_command(&Command::Push(Some(int(2))), &mut ctx)
        .unwrap();
...
fn remove_out_of_range_warns_and_stores_null() {
    ...
    inst.exec_command(
        &Command::Remove {
            from: RemoveFrom::Position(int(99)),
            to: Some(ToClause {
                cast: None,
                target: var("x"),
            }),
    ...
    assert!(matches!(
        ctx.warnings.as_slice(),
        [Warning::InvalidBufferPosition(_)]
    ));
}
...
fn switch_rotates_ninety_degrees_clockwise() {
    ...
    inst.exec_command(&Command::Switch(SwitchCond::Truthy(int(1))), &mut ctx)
        .unwrap();
...
fn switch_leaves_direction_alone_when_false() {
    ...
    inst.exec_command(&Command::Switch(SwitchCond::Truthy(int(0))), &mut ctx)
        .unwrap();
...
fn switch_falsy_inverts_the_test_not_the_rotation() {
    ...
    inst.exec_command(&Command::Switch(SwitchCond::Falsy(int(0))), &mut ctx)
        .unwrap();
...
fn switch_compares_against_the_dataspace_cell() {
    ...
    inst.exec_command(&Command::Switch(SwitchCond::Equals(int(7))), &mut ctx)
        .unwrap();
    assert_eq!(inst.tracer.direction, 90);

    inst.exec_command(&Command::Switch(SwitchCond::Equals(int(3))), &mut ctx)
        .unwrap();
...
fn switch_not_equals_is_the_complement() {
    ...
    inst.state.set_current_cell(7);
    inst.exec_command(&Command::Switch(SwitchCond::NotEquals(int(3))), &mut ctx)
        .unwrap();
    assert_eq!(inst.tracer.direction, 90);
}
...
fn go_with_an_uncastable_value_leaves_direction_alone() {
    ...
    inst.exec_command(
        &Command::Go {
            target: GoTarget::Value(str_lit("abc")),
            relative: false,
        },
        &mut ctx,
    )
...
fn storing_an_uncastable_value_to_the_dataspace_warns_and_stores_zero() {
    ...
    inst.exec_command(
        &Command::Store {
            source: StoreSource::Value(str_lit("abc")),
            target: None,
        },
        &mut ctx,
    )
    ...
    let scope = Scope {
        ...
        nodes: vec![
            ...
            Node {
                ...
                command: Command::Print(PrintTarget::Value(str_lit("hit"))),
            },
        ],
        checkpoints: vec![],
    };
...
#[test]
fn tangent_nodes_do_not_both_contain_a_point() {
    // At the default radius nodes two apart are tangent. An inclusive test put
    // the tracer inside both at once, firing both branch columns of a SWITCH.
    let midpoint = (8.0, 3.0);
    assert!(!point_in_circle(midpoint, (7.0, 3.0), 1.0));
    assert!(!point_in_circle(midpoint, (9.0, 3.0), 1.0));
}
